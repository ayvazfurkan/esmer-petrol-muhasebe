...
export default {
  ...
  methods: {
    get () {
      ipcRenderer.send('/chart/customerSegment', { branchId: this.getSession.userDetails.branchId })
      new Promise(function (resolve) {
        ipcRenderer.on('chartCustomerSegment', (event, response) => {
          resolve(response)
        })
      }).then(response => {
        const labels = _.map(response, 'segment')
