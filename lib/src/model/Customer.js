...
export default class Customer {
  ...
  get (where, rowCount) {
    ...
    if (where.name) {
      query.where('customer.name', 'like', '%' + where.name + '%')
      query.orWhere('customer.authorizedPersonName', 'like', '%' + where.name + '%')
    }
  ...
  getSummary (data, rowCount) {
    data = data || {}
    rowCount = rowCount || false
    const query = this.db('customerMoneyFlow as csmf')
      .select('csmf.id', 'csmf.amount', 'csmf.paymentType', 'csmf.description', 'csmf.oncreditId', 'csmf.creatorId', 'csmf.createDate', 'csmf.priceDifference', 'user.name', 'user.username', 'so.name as salesofficerName', 'so2.name as salesofficerName2', 'pym.name as paymentTypeName', 'drv.name as driverName', 'plt.plate as driverPlate')
      ...
      .leftJoin('salesofficer as so2', 'so2.id', 'csmf.salesofficerId')
      .leftJoin('paymentType as pym', 'pym.id', 'csmf.paymentType')
      .leftJoin('customerDriver as drv', 'drv.id', 'oc.customerDriverId')
      .leftJoin('customerPlate as plt', 'plt.id', 'oc.customerPlateId')
    if (data.customerId) {
  ...
  setParams (params) {
    return {
      ...
      forwardSalesDiscountRate: params.forwardSalesDiscountRate || '',
      ...
      oncreditDisabled: (params.oncreditDisabled && params.oncreditDisabled.id) || '0',
