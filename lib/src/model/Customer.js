...
export default class Customer {
  ...
  get (where, limit) {
    limit = limit || 500
    where = where || {}
    const query = this.db('customer').select('*').limit(limit)
    if (where.name) {
      query.where('name', 'like', where.name.concat('%'))
      query.orWhere('authorizedPersonName', 'like', where.name.concat('%'))
    }
    if (where.id) {
  ...
  getSummary (data, rowCount) {
    ...
    const query = this.db('customerMoneyFlow as csmf').select('csmf.id', 'csmf.amount', 'csmf.description', 'csmf.oncreditId', 'csmf.creatorId', 'csmf.createDate', 'user.name', 'user.username').leftJoin('user', 'csmf.creatorId', 'user.id')
    if (data.customerId) {
    ...
    if (!rowCount && data.dataPerPage) {
      if (!data.plateId && !data.driverId) {
        query.select(this.db.raw('(select sum(amount) from customerMoneyFlow WHERE id <= csmf.id) AS balance'))
      }
