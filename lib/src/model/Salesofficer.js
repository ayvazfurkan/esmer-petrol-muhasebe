...
export default class Salesofficer {
  ...
  static validateCreate (form) {
    ...
    if (_.isEmpty(form.phone) && form.phone.length < 5) {
      errors.phone = 'Bu alan +90 ile başlamalı ve 13 karakter olmalıdır.'
    }
    return {
  ...
  static validateUpdate (form) {
    ...
    if (!_.isEmpty(form.form.phone) && form.form.phone.length !== 13) {
      errors.phone = 'Bu alan +90 ile başlamalı ve 13 karakter olmalıdır.'
    }
    return {
      status: Object.keys(errors).length !== 0,
